...
export default function () {
  describe(descriptions.exportFn("cleanYarnLock"), function () {
    ...
    it("should remove all references to a repo in the yarn lockfile", async function () {
      ...
      expect(readJSONFile).to.be.calledOnceWithExactly(
        path.join(PROJECT_DIR, "config.poly")
      );
      ...
      expect(writeFileIfNotExist).to.be.calledOnceWithExactly(
        path.join(PROJECT_DIR, "yarn.lock"),
        NEW_LOCKFILE
      );
    });
