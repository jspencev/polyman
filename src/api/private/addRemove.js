...
export default async function addRemove(dependencies, type, config = {}, cwd) {
  ...
  for (const dep of dependencies) {
    if (config.local) {
      if (sameRepo) {
        ...
        throw Error(
          "Cannot modify local dependency when project not in the repository folder."
        );
      }
    } else {
      if (type === "add") {
  ...
  if (config.production) {
    ...
    possibleLocalDeps = myPack.localDependencies.concat(
      myPack.localDevDependencies
    );
  }
  ...
  for (const projectName of possibleLocalDeps) {
    ...
    if (sameRepo) {
      ...
      await moveFile(
        originalTarballPath,
        path.join(tmpTarballsDir, filename),
        true
      );
      ...
      if (config.force) {
        ...
        const localDep = fallback(
          myProject.local_dependencies[projectName],
          myProject.local_dev_dependencies[projectName]
        );
        if (hash !== localDep) {
          should = true;
        }
      ...
      if (config.install) {
        finalTarballPath = await findInMyDependencies(
          myProjectDir,
          projectName
        );
        should = true;
  ...
  try {
    ...
    if (config.production) {
      ...
      finalLocalDeps = myPack.localDependencies.concat(
        myPack.localDevDependencies
      );
    }
    ...
    if (sameRepo) {
      ...
      myProject.local_dependencies = _.pick(
        myProject.local_dependencies,
        myPack.localDependencies
      );
      myProject.local_dev_dependencies = _.pick(
        myProject.local_dev_dependencies,
        myPack.localDevDependencies
      );
...
async function getTarballPath(project) {
  if (!project) {
    throw Error(
      `We could not find project "${project.name}" in the repository file.`
    );
  }
