...
const OPTIONS = {
  ...
  tilde: {
    type: "boolean",
    description:
      "Installs most recent release of a package with the same minor version",
  },
};
...
export default async function cli(exec = false) {
  let yargs = require("yargs")
    ...
    .command(
      "add <dependency...>",
      "Add dependency(ies) to your project",
      function (yargs) {
    ...
    .command(
      ...
      "Add project(s) in this polyrepo as a dependency",
      function (yargs) {
    ...
    .command(
      "remove <dependency...>",
      "Remove dependency(ies) to your project",
      function (yargs) {
        yargs.positional("dependency", {
          description: "Project to add as dependency",
        });
    ...
    .command(
      "node <command...>",
      "Execute the node process. If --babel, executes with babel-node"
    );
  ...
  yargs = yargs.demandCommand().help();
  ...
  if (command === "init") {
    ...
    for (const arg of args) {
      if (
        !isOneOf(arg, "--inspect-brk", "--inspect", "--debug", "--debug-brk")
      ) {
        newArgs.push(arg);
      }
    }
    ...
    if (config.babel && isOneTruthy(argv.inspect, argv.inspectBrk)) {
      ...
      const tmpFilePath = path.join(
        fallback(appRootPath, process.cwd()),
        "./.poly/tmp"
      );
